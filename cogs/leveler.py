...
import os, re, aiohttp, functools
...
class Leveler:
    ...
    async def top10(self,ctx, global_rank:str = None):
        ...
        if global_rank == "global":
            ...
            await self.bot.say("Disabled Temporarily for Optimization.")
            return
        else:
            msg = "**Leaderboard for {}**\n".format(server.name)
            for userinfo in db.users.find({}):
    ...
    async def profilecolors(self, ctx, section:str, color:str):
        ...
        if color == "auto":
            if section == "exp":
                ...
                color_ranks = [random.randint(1,4)] # most prominent color
            elif section == "info":
    ...
    async def levelbg(self, ctx, *, image_name:str):
        ...
        if image_name in self.backgrounds["levelup"].keys():
            ...
            await self.bot.say("**That is not a valid bg. See available bgs at `{}background levelup`**".format(prefix[0]))
    ...
    async def listbgs(self, ctx, type:str = None):
        ...
        if not type:
            ...
            em.set_footer(text = "Suggest more at the owo! Official server: https://discord.gg/3HrrgHA")
            await self.bot.say(embed = em)
        else:
    ...
    async def draw_profile(self, user, server):
        ...
        text_u_fnt = ImageFont.truetype(font_unicode_file, 11)
